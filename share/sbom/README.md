> [!WARNING]
> 💥 Attention please! All (also this README.md) is highly experimental and Work In Progress stuff. Use with caution 🌟

# FreeBSD Lua Tools For SBOM (FLTS)

This directory contains tools for maintaining SBOMs for FreeBSD. Although they can work with other platforms and OS, they are mainly targeted at FreeBSD. As they aim to be easy to use, they remain somewhat rough and edgy in practice. This readme provides guidance on their purpose and usage.

Currently, four Lua scripts are available:
- `flts-convert_app_csv_to_yaml.lua` — Converts CSV (separated with '|') to `database.yaml`. Though largely deprecated, it remains for historical reasons.
- `flts-convert_license_to_database.lua` — Adds licenses to `database.yaml`. This tool is also more or less deprecated.
- `flts-create_pc_files.lua` — A versatile script that can generate `.pc`-files from `database.yaml`, among other tasks.
- `flts-read_scancode_yaml.lua` — Converts `scancode-toolkit` JSON format into a simplified JSON for easier SBOM creation.

## Why Does FreeBSD Need SBOM?

We live in a vastly different world than when FreeBSD was created, and it’s safe to say the landscape will continue evolving rapidly over the coming years and decades. Supply chain attacks have become commonplace rather than an oddity. Regulations have also tightened as digital services have become the backbone of societies. For example, the EU Cyber Resilience Act (CRA) now mandates SBOMs for any commercial service with network-accessible components. The United States has similarly begun enforcing stricter laws for internet-based services and connected devices.

As FreeBSD silently enables internet services—routing billions of packets per second to and from the Internet—it is critical to provide an SBOM that downstream users and base users can rely on and extend in the future.

Currently, one key piece of the puzzle, `pkgconf`, is included in FreeBSD’s source tree, and `.pc`-files are under active review [here](https://reviews.freebsd.org/D56474).

### `flts-create_pc_files.lua`
This script generates multiple output formats from (`database.yaml`)[https://raw.githubusercontent.com/FreeBSDFoundation/lpha-omega-beach-cleaning/refs/heads/database_yml-add-more-apps/database.yml), including:
- **pkgconf** — Writes pkgconf files (e.g., `pkgconfig/accton.pc`).
- **markdown** — Exports database info as a Markdown table for GitHub or similar platforms.
- **deps** — Validates dependencies in `database.yaml`.
- **apps** — Lists applications.
- **ucl** — Converts YAML entries to UCL files (named after the application).
- **makefile** — Generates Makefile entries from YAML.

For license text information, the file [`data/scancode_all.json`](data/scancode_all.json) is required. It contains curated `scancode` output for bin, sbin, usr.bin, and usr.sbin.

### `flts-read_scancode_yaml.lua`
TBD

### `flts-convert_app_csv_to_yaml.lua` *(Deprecated)*
TBD

### `flts-read_scancode_yaml.lua` *(Deprecated)*
TBD
